...
class Ui_MainWindow(object):
    def InsertData1(self):
        pr_id = [self.tableWidget.item(row, 0).text() for row in range(self.tableWidget.rowCount())]
        ...
        name = [self.tableWidget.item(row, 3).text() for row in range(self.tableWidget.rowCount())]
        alias = [self.tableWidget.item(row, 4).text() for row in range(self.tableWidget.rowCount())]
        appearance = [self.tableWidget.item(row, 5).text() for row in range(self.tableWidget.rowCount())]
        creator = [self.tableWidget.item(row, 6).text() for row in range(self.tableWidget.rowCount())]
        print(pr_id[0], pub_id[0], team_id[0], name[0], alias[0], appearance[0], creator[0])
        query1 = f"INSERT INTO protagonist (Protagonist_id, Publisher_id, Team_id, Name, Alias, Appearance, Creator) VALUES ({int(pr_id[0])}, {int(pub_id[0])}, {int(team_id[0])}, '{name[0]}', '{alias[0]}', {int(appearance[0])}, '{creator[0]}');"
    ...
    def InsertData2(self):
        pr_id1 = [self.tableWidget_2.item(row, 0).text() for row in range(self.tableWidget_2.rowCount())]
        ...
        ant = 2052
        query2 = f"INSERT INTO stats (Protagonist_id, Antagonist_id, Intelligence, Strength, Speed, Durability, Power, Combat, Tier) VALUES ({int(pr_id1[0])}, {ant},  {int(intelligence[0])}, {int(strength[0])}, '{int(speed[0])}', '{int(durability[0])}', {int(power[0])}, {int(combat[0])}, '{int(tier[0])}');"
        try:
            mydb1 = mc.connect(
                host="localhost",
                user="root",
    ...
    def retranslateUi(self, MainWindow):
        ...
        item.setText(_translate("MainWindow", "Alliance"))
